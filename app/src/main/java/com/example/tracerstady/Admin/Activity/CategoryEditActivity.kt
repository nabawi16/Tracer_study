...
class CategoryEditActivity : AppCompatActivity(), OnMapReadyCallback {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        cekMap.setOnClickListener {
            ...
            if (lat.isEmpty() || lon.isEmpty()){
                Toast.makeText(this, "Masukkan lat dan lon", Toast.LENGTH_SHORT).show()
            }else{
        ...
        saveBtn.setOnClickListener {
            ...
            val id_category1 = nameEditText.text.toString()
            val lat = edt_lat.text.toString()
            val lon = edt_lon.text.toString()
    ...
    private fun saveCategory(name1:String, id_category1: String, name: String, lat: String, lon: String) {
        ...
        storage = FirebaseStorage.getInstance().reference.child("name").child(name)
        ...
        if (photo_location != null){
            ...
            mStorageReference.putFile(photo_location!!)
                ...
                .addOnSuccessListener {
                    ...
                    ref.orderByChild("lokasi").equalTo(name1).addListenerForSingleValueEvent(object :
                        ValueEventListener {
                        override fun onDataChange(p0: DataSnapshot) {
                            if (p0.exists()){
                                for (datasnapshot1: DataSnapshot in p0.children) {
                                    val id_working = datasnapshot1.key
                                    ref.child(id_working!!).child("lokasi").setValue(name)
                                }
                            }
                        }
